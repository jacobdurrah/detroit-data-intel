...
function getDealScore(deal) {
  return deal.motivation_score ?? deal.score ?? 0;
}
...
function renderPipelineStats() {
  ...
  const avgScore = total > 0 ? (APP.pipeline.reduce((s, d) => s + getDealScore(d), 0) / total).toFixed(1) : 0;
  const withMatches = APP.pipeline.filter(d => d.match_count > 0).length;
  const highScore = APP.pipeline.filter(d => getDealScore(d) >= 50).length;
...
function renderPipelineTable() {
  ...
  APP.pipeline.forEach(deal => {
    const dealScore = getDealScore(deal);
    const scoreClass = dealScore >= 50 ? 'score-high' :
                       dealScore >= 30 ? 'score-mid' : 'score-low';
    ...
    html += '<tr>' +
      '<td><span class="score-badge ' + scoreClass + '">' + dealScore + '</span></td>' +
  ...
  APP.pipeline.forEach(deal => {
    ...
    html += '<div class="data-card">' +
      '<div class="data-card-header">' +
        ...
        '<span class="score-badge ' + scoreClass + '">' + dealScore + '</span>' +
      '</div>' +
